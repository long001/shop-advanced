...
class ProductsController extends Controller
{
    ...
    public function index_1(Request $request)
    {
        ...
        return view('products.index', [
            ...
            // 将类目树传递给模板文件
            // 换为使用 ViewComposer 注入 categoryTree 变量
    ...
    public function index(Request $request)
    {
        ...
        // // 是否有提交 类目 参数，如果有就赋值给 $category 变量
        // if ($request->input('category_id') && $category = Category::find($request->input('category_id'))) {
        //     if ($category->is_directory) {
        ...
        //     // Mysql 的 in 查询并不会按照参数的顺序把结果返回
        //     ->whereIn('id', $productIds)
        //     // orderByRaw 可以让我们用原生的 SQL 来给查询结果排序
        //     // "FIND_IN_SET(id, '1,6,7,14')"
    ...
    // 保留，但不再使用的。可做参考
    public function show_1(Product $product, Request $request)
    {
    ...
    public function show(Product $product, Request $request, ProductService $service)
    {
        ...
        // 最后别忘了注入到模板中
        return view('products.show', [
            'product' => $product, // 商品本身
            'favored' => $favored, // 是否已收藏
            'reviews' => $reviews, // 评价
